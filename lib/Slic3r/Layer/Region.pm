...
# collection of Slic3r::Layer::Region::Island objects holding perimeters and fills
has 'islands' => (is => 'rw', default => sub { [] });
...
sub make_perimeters {
    ...
    foreach my $surface (@surfaces) {
        # initialize island (should this be somewhere else?)
        push @{$self->islands}, my $island = Slic3r::Layer::Region::Island->new(
            region => $self,
        );
        ...
        # create one more offset to be used as boundary for fill
        {
            ...
            push @{ $island->fill_surfaces }, @fill_boundaries;
        }
    ...
    foreach my $island (@perimeters) {
        ...
        $self->_add_perimeter($island, $holes[$_], $is_external{$_} ? EXTR_ROLE_EXTERNAL_PERIMETER : undef)
            for reverse 0 .. $#holes;
        for my $depth (reverse 0 .. $#$island) {
            my $role = $depth == $#$island ? EXTR_ROLE_CONTOUR_INTERNAL_PERIMETER
                ...
                : EXTR_ROLE_PERIMETER;
            $self->_add_perimeter($island, $_, $role) for map $_->contour, @{$island->[$depth]};
        }
    }
    ...
    if ($self->layer->id == 0 && $Slic3r::Config->brim_width > 0) {
        @{$island->perimeters} = reverse @{$island->perimeters};
    }
    
    # add thin walls as perimeters
    push @{ $island->perimeters }, Slic3r::ExtrusionPath::Collection->new(paths => [
        map {
...
sub _add_perimeter {
    my $self = shift;
    my ($island, $polygon, $role) = @_;
    
    return unless $polygon->is_printable($self->perimeter_flow->scaled_width);
    push @{ $island->perimeters }, Slic3r::ExtrusionLoop->pack(
        polygon         => $polygon,
